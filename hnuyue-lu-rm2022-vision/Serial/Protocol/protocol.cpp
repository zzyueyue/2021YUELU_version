...
*/
    ...
    bool Protocol::decode(const std::string& s,VisionRecvData & decoded_data,IMUInfo &abs_angle)
    {
        static char ch_arr[255];
        static uint16_t msg;
        ...
        // 
        abs_angle.pitch=f_arr[0];
        abs_angle.yaw=f_arr[1];
        // just modifiy here , use displacement operation to get state code
        decoded_data=  VisionRecvData((SelfColor)(msg & 0xf),           // self_color
                                        (msg >> 4) & 0xf,                            // actuator_id
                                        static_cast<hnurm::WorkMode>((msg >> 8) & 0xf),        // mode: 0 manual, 1 auto_shoot, 2 auto_windmill, 3 auto_save, 4 auto_grab
                                      (BulletSpeed)((msg >> 4) & 0xf));   // trace: 0 off, 1 on
    }




void Protocol::get_protocol_send_data_vision(uint16_t send_id,	 	       
